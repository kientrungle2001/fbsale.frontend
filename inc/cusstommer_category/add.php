...
<script type="text/javascript">
	...
	$("#formData").submit(function(e) {
		...
		if($(this).attr('datatype') == 'add'){
			var url = "<?php echo FBSALE_API_URL ?>/coremenus/"; // the script where you handle the form input.
	   		 $.ajax({
	           type: "POST",
	           url: url,
			...
			var url = "<?php echo FBSALE_API_URL ?>/coremenus/"+id; // the script where you handle the form input.
	   		 $.ajax({
	           type: "PATCH",
	           url: url,
	           data: $("#formData").serialize(), // serializes the form's elements.
